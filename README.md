...
- 2x `effiddy_case_bottom.stl`, which is the bottom part of the case - same story, use the provided 3mf files for that: `effiddy_case_bottom_left.3mf` and `effiddy_case_bottom_right.3mf`. These are just mirrored versions. **Note**: The `effiddy_case_bottom.stl` generated by ergogen - due to its complexity - is cursed and a little bit broken. The shape is not closed, there are multiple naked edges, planar and non-planar holes as well as degenerate faces. Despite that, I had no trouble printing it in Bambustudio.
